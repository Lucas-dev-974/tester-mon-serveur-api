...
class GameViewsetTestCase(TestCase):
    ...
    def test_update_game(self):
        ...
        response = self.client.put(
            f"http://127.0.0.1:8000/api/game/{self.game1.id}/", data
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_game(self):
        response = self.client.delete(
            f"http://127.0.0.1:8000/api/game/{self.game1.id}/"
        )
    ...
    def test_retrieve_game_error(self):
        response = self.client.get(
            f"http://127.0.0.1:8000/api/game/5"
        )  # Missing data in bdd
    ...
    def test_create_game_error_2(self):
        data = {
            ...
            "editor": "New Editor",
            "nb_players": "yuyuy",
        }  # nb_players string instead of integer
        response = self.client.post("http://127.0.0.1:8000/api/game/", data)
        self.assertEqual(response.status_code, 400)
